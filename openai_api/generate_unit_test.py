...
def unit_test_from_function(
    ...
    code_model: str = "gpt-3.5-turbo", # "code-davinci-002",  # if you don't have access to code models, you can use text models here instead
...
#     unit_test_response = openai.Completion.create(
    unit_test_response = openai.ChatCompletion.create(
        model=code_model,
        messages = [
            {'role': 'user', 'content': full_unit_test_prompt}
                  ],
        stop="```",
        max_tokens=max_tokens,
        temperature=temperature,
    ...
    return unit_test_completion, unit_test_response


# example_function = """def is_palindrome(s):
    # return s == s[::-1]"""

example_function = """def fizz_buzz(n):
    out_str = ""
    if n%3 == 0:
        out_str += "Fizz"
    if n%5 == 0:
...
output_text, response = unit_test_from_function(example_function, print_text=True)
